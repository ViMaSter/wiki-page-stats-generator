...
public abstract class GeneratePageStatsMarkdown
{
    ...
    public static void Execute([FromService] HttpClient httpClient, [FromService] DateTime now, string azurePAT,
        string organization, string project, string wikiIdentifier, string outputPath)
    {
        httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic",
            Convert.ToBase64String(Encoding.UTF8.GetBytes($":{azurePAT}")));
        ...
        do
        {
            ...
            response.Headers.TryGetValues("x-ms-continuationtoken", out var continuationTokenArray);
            continuationToken = continuationTokenArray?.FirstOrDefault();

            var body = response.Content.ReadAsStringAsync().Result;
        
            var wikiPageDetail = JsonConvert.DeserializeObject<WikiPageDetail>(body)!;
            foreach (var value in wikiPageDetail.value)
            {
                if (value.viewStats == null)
                {
                    continue;
                }
